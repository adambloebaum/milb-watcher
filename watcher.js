...
async function checkAndNotify() {
  ...
  async function getTodaysGames() {
    try {
      ...
      const response = await fetch(`${SCHEDULE_ENDPOINT}${LEAGUE_LEVEL}`);
      ...
      if (data.dates && data.dates.length > 0) {
        for (const date of data.dates) {
          for (const game of date.games) {
            ...
            if (TEAM_ID === 'TEAM_ID_HERE' || 
                game.teams.away.team.id === TEAM_ID || 
    ...
    try {
      ...
      for (const team of teams) {
        ...
        for (const player of activePlayers) {
          if (player.person && player.person.id && player.person.id.toString() === PLAYER_ID) {
        ...
        if (team.substitutions) {
          for (const sub of team.substitutions) {
            if (sub.incomingPlayer && sub.incomingPlayer.id && sub.incomingPlayer.id.toString() === PLAYER_ID) {
              if (!playerInGame) {
                // Player just entered the game
                playerInGame = true;
                notifiedGames.add(gameId);
        ...
        for (const player of benchPlayers) {
          if (player.id && player.id.toString() === PLAYER_ID) {
        ...
        if (liveFeedResponse.ok) {
          ...
          if (liveFeedData.plays && liveFeedData.plays.allPlays) {
            for (const play of liveFeedData.plays.allPlays) {
              ...
              if (play.matchup && 
                  ((play.matchup.batter && play.matchup.batter.id.toString() === PLAYER_ID) || 
                   (play.matchup.pitcher && play.matchup.pitcher.id.toString() === PLAYER_ID))) {
                if (!playerInGame) {
                  playerInGame = true;
                  notifiedGames.add(gameId);
                  return true;
              ...
              if (play.playEvents) {
                for (const event of play.playEvents) {
                  if (event.player && event.player.id && event.player.id.toString() === PLAYER_ID) {
                    if (!playerInGame) {
                      playerInGame = true;
                      notifiedGames.add(gameId);
                      return true;
  ...
  async function sendNotification(gameId) {
    try {
      ...
      if (twilioClient && config.TWILIO_PHONE_NUMBER && config.PHONE_NUMBERS && config.PHONE_NUMBERS.length > 0) {
        console.log(`Sending SMS alerts to ${config.PHONE_NUMBERS.length} recipients...`);
        ...
        const messagePromises = config.PHONE_NUMBERS.map(phoneNumber => {
          return twilioClient.messages.create({
            body: `🎉 ${config.PLAYER_NAME} has entered the game! (${gameInfo})`,
      ...
      } else if (config.PHONE_NUMBERS && config.PHONE_NUMBERS.length > 0) {
        console.log('⚠️ SMS notifications requested but Twilio not properly configured.');
      }
    } catch (error) {
  ...
  async function checkAndNotify() {
    try {
      ...
      // Reset player status at beginning of check
      playerInGame = false;
      
      const games = await getTodaysGames();
      console.log(`Found ${games.length} relevant games for league level ${LEAGUE_LEVEL}`);
      
      if (games.length === 0) {
        console.log('No live or upcoming games found. Will check again later.');
        return;
      }
      ...
      for (const game of games) {
        if (game.status === 'Live') {
          console.log(`Checking game ${game.id} (${game.detailedState})...`);
          const playerEntered = await checkPlayerInGame(game.id, game.link);
          
          if (playerEntered) {
            sendNotification(game.id);
          }
        } else {
          console.log(`Game ${game.id} not started yet (${game.detailedState}). Will check later.`);
        }
      }
      
      if (!playerInGame) {
        console.log(`${PLAYER_NAME} has not entered any games yet. Will check again later.`);
      }
    } catch (error) {
      console.error('Error in check and notify process:', error.message);
    }
